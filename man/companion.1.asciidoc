...
WARNING
-------
This man page may be less up-to-date than the usage text in companion (`companion -h`).

DESCRIPTION
-----------
companion is a compositor based on Dana Jansens' version of xcompmgr (which itself was written by Keith Packard). It includes some improvements over the original xcompmgr, like window frame opacity and inactive window transparency.
...
*--config* 'PATH'::
	Look for configuration file at the path. See *CONFIGURATION FILES* section below for where companion looks for a configuration file by default.
...
*--refresh-rate* 'REFRESH_RATE'::
	Specify refresh rate of the screen. If not specified or 0, companion will try detecting this with X RandR extension.
...
*--sw-opti*::
	Limit companion to repaint at most once every 1 / 'refresh_rate' second to boost performance. Experimental. This should not be used with *--vsync* as *--vsync* essentially does *--sw-opti*'s job already, unless you wish to specify a lower refresh rate than the actual value.
...
----
+
May also be one of the predefined kernels: `3x3box` (default), `5x5box`, `7x7box`, `3x3gaussian`, `5x5gaussian`, `7x7gaussian`, `9x9gaussian`, `11x11gaussian`. All Guassian kernels are generated with sigma = 0.84089642 . You may use the accompanied `companion-convgen.py` to generate blur kernels.
...
CONFIGURATION FILES
-------------------
companion could read from a configuration file if libconfig support is compiled in. If *--config* is not used, companion will seek for a configuration file in `$XDG_CONFIG_HOME/companion.conf` (`~/.config/companion.conf`, usually), then `~/.companion.conf`, then `companion.conf` under `$XDG_DATA_DIRS` (often `/etc/xdg/companion.conf`).

companion uses general libconfig configurtion file format. A sample configuration file is available as `companion.sample.conf` in the source tree. Most commandline switches each could be replaced with an option in configuration file, thus documented above. Window-type-specific settings are exposed only in configuration file and has the following format:

------------
wintypes:
{
  WINDOW_TYPE = { fade = BOOL; opacity = FLOAT; focus = BOOL; };
};
------------

'WINDOW_TYPE' is one of the 15 window types defined in EWMH standard: "unknown", "desktop", "dock", "toolbar", "menu", "utility", "splash", "dialog", "normal", "dropdown_menu", "popup_menu", "tooltip", "notify", "combo", and "dnd". "fade" and controls window-type-specific fade settings. "opacity" controls default opacity of the window type. "focus" controls whether the window of this type is to be always considered focused. (By default, all window types except "normal" and "dialog" has this on.)

SIGNALS
-------

* companion reinitializes itself upon receiving `SIGUSR1`.

D-BUS API
---------

It's possible to control companion via D-Bus messages, by running companion with *--dbus* and send messages to `com.github.aaronhamilton.companion.<DISPLAY>`. `<DISPLAY>` is the display used by companion, with all non-alphanumeric characters transformed to underscores. For `DISPLAY=:0.0` you should use `com.github.aaronhamilton.companion._0_0`, for example.
...
+
------------
$ companion --config /dev/null
------------

* Make inactive windows 80% transparent, making frame 80% transparent, don't fade on window open/close, enable software optimization, and fork to background:
+
------------
$ companion -b -i 0.8 -e 0.8 --no-fading-openclose --sw-opti
------------

* Enable OpenGL SGI_swap_control VSync with GLX backend:
+
------------
$ companion --backend glx --vsync opengl-swc
------------

BUGS
----
Please report any you find to <https://github.com/aaronhamliton/companion> .

AUTHORS
-------
xcompmgr, originally written by Keith Packard, with contributions from Matthew Allum, Eric Anholt, Dan Doel, Thomas Luebking, Matthew Hawn, Ely Levy, Phil Blundell, and Carl Worth. companion by Aaron Hamilton, based almost entirely on the work of Christopher Jeffrey, Dana Jansens, with contributions from Richard Grenville.

RESOURCES
---------
Homepage: <https://github.com/aaronhamilton/companion>

SEE ALSO
--------
*xcompmgr*(1), link:companion-trans.html[*companion-trans*(1)]
